...
def load_hthi_data(datasource_path: os.path, label_index: int, use_pha: bool):
    # amp(180, seq_len), phase(180, seq_len), label(1, 1)
    datasource = {
        'data': list(),
        'label': list(),
    }
    dir_name = 'I%d' % (label_index + 1)
    file_list = os.listdir(os.path.join(datasource_path, dir_name))
    random.shuffle(file_list)
    for j in tqdm(range(len(file_list))):
        mat_file = scio.loadmat(os.path.join(datasource_path, dir_name, file_list[j]))
        amp = np.expand_dims(mat_file['amp'], 0)
        pha = np.expand_dims(mat_file['phase'], 0)
        label = mat_file['label'] - 1
        if use_pha:
            data = np.concatenate((amp, pha), axis=1)
        else:
            data = amp
        datasource['data'].append(data)
        datasource['label'].append(label)
    return datasource


def split_train_test(datasource_path: os.path, use_pha: bool, test_ratio=0.1):
    train_mat = {
        'data': [],
        'label': [],
    }
    ...
    for i in range(n_class):
        datasource = load_hthi_data(datasource_path, i, use_pha)
        n_sample = len(datasource['label'])
        assert n_sample == len(datasource['data'])
        train_mat['data'].extend(datasource['data'][:int(n_sample * (1 - test_ratio))])
        train_mat['label'].extend(datasource['label'][:int(n_sample * (1 - test_ratio))])
        test_mat['data'].extend(datasource['data'][int(n_sample * (1 - test_ratio)):])
        test_mat['label'].extend(datasource['label'][int(n_sample * (1 - test_ratio)):])
...
if __name__ == '__main__':
    ...
    for i in range(random_times):
        ...
        train_mat, test_mat = split_train_test(datasource_path, use_pha)
        ...
        for segment_length_rate in segment_length_rates:
            for overlap_length_rate in overlap_length_rates:
                ...
                save_mat(os.path.join(datasource_path,
                                      'train_dataset_%d_%s_%.2f_%.2f.h5' % (i, 'pha' if use_pha else 'none',
                                                                            segment_length_rate,
                                                                            overlap_length_rate)),
                         train_result)
                save_mat(os.path.join(datasource_path,
                                      'test_dataset_%d_%s_%.2f_%.2f.h5' % (i, 'pha' if use_pha else 'none',
                                                                           segment_length_rate,
                                                                           overlap_length_rate)),
                         test_result)
