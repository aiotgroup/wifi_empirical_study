...
class HARLNLDatasetConfig(DatasetConfig):
    def __init__(self, datasource_path: os.path, index: int, segment_length_rate: float, overlap_length_rate: float):
        super(HARLNLDatasetConfig, self).__init__('harlnl', datasource_path)
        self.sample_rate = 254
        self.index = index
        self.segment_length_rate = segment_length_rate
        self.overlap_length_rate = overlap_length_rate


def load_harlnl_data(config: HARLNLDatasetConfig):
    train_mat = load_mat(os.path.join(config.datasource_path,
                                      'train_dataset_%d_%.2f_%.2f.h5' % (
                                          config.index,
                                          config.segment_length_rate,
                                          config.overlap_length_rate)))
    test_mat = load_mat(os.path.join(config.datasource_path,
                                     'test_dataset_%d_%.2f_%.2f.h5' % (
                                         config.index,
                                         config.segment_length_rate,
                                         config.overlap_length_rate)))
...
class HARLNLDataset(Dataset):
    def __init__(self, mat_data):
        super(HARLNLDataset, self).__init__()

        logger.info('加载HARLNL数据集')
...
if __name__ == '__main__':
    ...
    train_data, test_data = load_harlnl_data(HARLNLDatasetConfig(datasource_path, 0, 1, 0.75))
    train_dataset = HARLNLDataset(train_data)
    test_dataset = HARLNLDataset(test_data)
